...
def home(request):
    ...
    search_class = Search('')
    favorites_series_airing_this_week=[]
    favorite_seriesid= [int(item) for item in this_user.favorites[1:-1].split(',')]
    print(favorite_seriesid)
    test=Serie.objects.filter(id=69629)
    for j in range(len(favorite_seriesid)):
        dico_serie={}
        serie=Serie.objects.filter(id=favorite_seriesid[j])[0]
        if int(serie.alert)<7:
            dico_serie["id"]=serie.id
            dico_serie["name"]=serie.name
            dico_serie["poster_path"]=serie.poster_path
            favorites_series_airing_this_week.append(dico_serie)
...
def serieinfo(request,serie_id):
    ...
    try:
        ...
        error_message="The TV show you requested could not be found."
        return render(request, 'webapp/error.html',locals())        
...
def seasoninfo(request,serie_id,season_number):
    ...
    try:
        ...
        error_message="The info you are looking for could not be found."
        return render(request, 'webapp/error.html',locals())
    season_info = search_class._get_attributes_for_season(serie_id,season_number)
